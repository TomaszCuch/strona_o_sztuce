...
function scrollToElementAndPauseHeader(target) {
  ...
  // Czekamy aż przewijanie się zakończy
  let prevY = window.scrollY;
  let stableFrames = 0;

  function check() {
    const y = window.scrollY;
    stableFrames = Math.abs(y - prevY) < 0.5 ? stableFrames + 1 : 0;
    prevY = y;
...
// Animacje dla article z artystami
...
document.addEventListener("focusin", (e) => {
  if (e.target.matches(':focus-visible') && e.target.classList.contains("artist-row")) 
  {
...
// Przewijanie za pomocą tab artystów w górę bez zasłaniającego headera
document.addEventListener("focusin", (e) => {
  const row = e.target.closest(".artist-row.visible");
  if (!row) return;
