...
const messages = {
    required: isEn ? "Please fill out this field." : "To pole jest wymagane.",
    email: isEn ? "Please enter a valid email address (e.g. name@example.com)." : "Wpisz poprawny adres e-mail (np. imie@przyklad.pl).",
};
...
if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
        ...
        if (isFormValid) {
            const parentSection = contactForm.parentElement;
            const messageTitle = isEn ? "Thank you!" : "Dziękujemy!";
            const messageBody = isEn ? "Your message has been sent successfully." : "Twoja wiadomość została wysłana pomyślnie.";
            contactForm.style.display = 'none';
            const successDiv = document.createElement('div');
            successDiv.className = 'success-message';
            successDiv.innerHTML = `<span class="success-icon">✓</span><h3>${messageTitle}</h3><p>${messageBody}</p>`;
            parentSection.appendChild(successDiv);
        }
    });
}


// Wyśrodkowanie dla klawisza tab dla zdjęć artystów
document.addEventListener("focusin", (e) => {
    if ((e.target.matches(':focus-visible') && e.target.classList.contains("contact_cell")) ||
